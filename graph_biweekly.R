...
graph_data = graph_data1 %>% filter(Date_of_run > "2023-1-25")

z =ggplot(graph_data, aes(as.Date(Date_of_run), per, fill = 
                            pango_lineage, label = pango_lineage)) + geom_bar(stat='identity') +
  geom_text(size = 3.6, position = position_stack(vjust = 0.5)) +
