...
task compile {
  ...
  echo -e "GISAID Virus Name\tSamplename\tNumber of N" > ~{repository}_batched_samples.tsv
  echo -e "GISAID Virus Name\tSamplename\tNumber of N" > ~{repository}_excluded_samples.tsv
  ...
  for index in ${!assembly_array[@]}; do
   ...
   # remove samples that exceed N threshold
  for index in ${!assembly_array[@]}; do
    assembly=${assembly_array[$index]}
    assembly_header=$(grep -e ">" $assembly | sed 's/\s.*$//' | sed 's/>//g' )
    echo $assembly_header
    meta=${meta_array[$index]}
    samplename=${samplename_array[$index]}
    N_num=${number_N_array[$index]}
    
  # remove samples from array if number_N exceedes N_threshold
    if [ "${N_num}" -gt "~{N_threshold}" ]; then
      echo "$assembly removed: number_N (${N_num}) exceeds N_threshold (~{N_threshold})"
      echo -e "$assembly_header\t$samplename\t$N_num" >> ~{repository}_excluded_samples.tsv
    else
      passed_assemblies=( "${passed_assemblies[@]}" "$assembly")
      passed_meta=( "${passed_meta[@]}" "$meta")
      echo "$assembly added to batch:  number_N (${N_num}) within N_threshold (~{N_threshold})"
      echo -e "$assembly_header\t$samplename\t$N_num" >> ~{repository}_batched_samples.tsv
    fi
...
workflow mercury_batch {
	...
	call compile as genbank_compile{
		input:
	...
	call compile as gisaid_compile{
		input:
