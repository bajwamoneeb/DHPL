...
Terra <- read_tsv("sample - 2023-04-13T143407.323.tsv")
...
barcodes_filtered = data.frame(Terra_filtered$`entity:sample_id`, Terra_filtered$purpose_of_sequencing)
...
submission_id=paste(lims_metadata.2$iso_state,"DHSS",lims_metadata.2$`entity:sample_id`,sep="-")
...
for(i in 1:nrow(lims_metadata.2.1)) {
  if (grepl('Nemours',lims_metadata.2.1$depscription[i], ignore.case = T)==T){
...
write(unique(not_found$`entity:sample_id`), "barcodes_notfound.txt")
...
metadata$collection_date<-strftime(mdy_hms(metadata$collection_date), "%Y-%m-%d")
...
write_tsv(metadata, "database_cl_metadata.tsv")
