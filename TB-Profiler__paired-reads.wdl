...
workflow TB_Profiler {
  ...
  output {
   ...
   String	region_median_depth= tb_profiler.region_median_depth
...
task tb_profiler {
  ...
  command <<<
    ...
    pct_reads_mapped=$(cut -f6 ~{sample}.txt | grep -v "pct_reads_mapped")
    num_reads_mapped=$(cut -f7 ~{sample}.txt | grep -v "num_reads_mapped")
    region_median_depth=$(cut -f5 ~{sample}.txt | grep -v "region_median_depth")
    ...
    pyrazinamide=$(cut -f13 ~{sample}.txt | grep -v "pyrazinamide")
    ethambutol=$(cut -f12 ~{sample}.txt | grep -v "ethambutol")
    ...
    kanamycin=$(cut -f23 ~{sample}.txt | grep -v "kanamycin")
    capreomycin=$(cut -f22 ~{sample}.txt | grep -v "capreomycin")
    ethionamide=$(cut -f25 ~{sample}.txt | grep -v "ethionamide")
    para_aminosalicylic_acid=$(cut -f27 ~{sample}.txt | grep -v "para-aminosalicylic_acid")
    cycloserine=$(cut -f24 ~{sample}.txt | grep -v "cycloserine")
    linezolid=$(cut -f30 ~{sample}.txt | grep -v "linezolid")
    bedaquiline=$(cut -f29 ~{sample}.txt | grep -v "bedaquiline")
    clofazimine=$(cut -f26 ~{sample}.txt | grep -v "clofazimine")
    delamanid=$(cut -f28 ~{sample}.txt | grep -v "delamanid")
    ...
    echo $region_median_depth | tee region_median_depth
  ...
  output {
   ...
   String	region_median_depth= read_string("region_median_depth")
